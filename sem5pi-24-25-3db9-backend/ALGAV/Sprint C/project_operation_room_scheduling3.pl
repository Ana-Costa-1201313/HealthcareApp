...
:- http_handler('/genetic_Algorithm'					, run_genetic_Algorithm					, [method(get) ]).
...
run_operations_to_rooms_automatic(Request) :-
    ...
    % Return the response with the assigned surgeries
    Response = json{
        'Message': 'Surgeries allocated successfully',
        'AssignedOperations': AssignmentList
    },
...
run_genetic_Algorithm(Request):-
    ...
    (
        (
			full(DayString), !,
			findall((Staff,DayString, A), agenda_staff2(Staff, DayString, A), AgendaS),
			to_string_list(AgendaS, AgendaSStr),
			findall((OR, DayString, A), agenda_operation_room2(OR, DayString, A), L),
			to_string_list(L,Lstr),
			Response = json{
				'AgendasDoctors': AgendaSStr,
				'AgendaRooms': Lstr
			},
			reply_json(Response)
		); 
        (
            Response2 = json{'try again':'try again', 'Day':DayString}, 
            reply_json(Response2)
        )
	).
%    ), 
%	findall((Staff,DayString, A), agenda_staff2(Staff, DayString, A), AgendaS),
%	to_string_list(AgendaS, AgendaSStr),
%	findall((Staff, DayString, A), agenda_operation_room2(Staff, DayString, A), L),
%	to_string_list(L,Lstr),
%    Response = json{
%        'AgendasDoctors': AgendaSStr,
%        'AgendaRooms': Lstr
...
surgery_total_time(Type, TotalTime) :-
    surgery(Type, Prep, Duration, Recovery),
